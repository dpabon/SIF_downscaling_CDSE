...
```{python}
# Right after loading STAC
cube_SIF_test = connection.load_stac(
...
print("SIF cube after loading:")
print(cube_SIF_test.to_json())
```
 

 ```{python}
 def check_bands(cube, name):
    try:
        pg = cube.to_json()
        print(f"\n{name}:")
        print(f"Process graph nodes: {list(pg['process_graph'].keys())}")
    except:
        print(f"\n{name}: Unable to inspect")
 ```
...
```{python}
check_bands(cube_SIF_original_median, "SIF after load")

# After reduce
cube_SIF_original_median = cube_SIF_original_median.reduce_temporal(reducer="median")
check_bands(cube_SIF_original_median, "SIF after reduce_temporal")

# For LST
cube_LST_median_low = cube_LST_median.resample_cube_spatial(
    target=cube_SIF_original_median, method="bilinear"
)
check_bands(cube_LST_median_low, "LST after resample (before add_dimension)")

cube_LST_median_low = cube_LST_median_low.add_dimension(name="bands", label="LST")
check_bands(cube_LST_median_low, "LST after add_dimension")

# First merge
dataset_SIF_low = cube_SIF_original_median.merge_cubes(cube_LST_median_low)
check_bands(dataset_SIF_low, "After first merge (SIF + LST)")
```
...
# We setup a phyisical filter in general we considered that values for June below 10 degrees are not realistic

lst_band = cube_LST_masked.band("LST")
mask = lst_band <= (10 + 273)
cube_LST_masked_2 = lst_band.mask(mask)
...
cube_LST_median = cube_LST_masked_2.reduce_temporal(reducer="median")
...
```{python}
print("=== Cube structures before resampling ===")
print("SIF cube metadata:")
print(cube_SIF_original_median.metadata)
print("\nLST cube metadata:")
print(cube_LST_median.metadata)
print("\nOTCI cube metadata:")
print(cube_OTCI_median.metadata)
print("\nIWV cube metadata:")
print(cube_IWV_median.metadata)

# After resampling and add_dimension
print("\n=== Cube structures after resampling and add_dimension ===")
print("LST_low bands:")
print(cube_LST_median_low.metadata)
print("\nOTCI_low bands:")
print(cube_OTCI_median_low.metadata)
print("\nIWV_low bands:")
print(cube_IWV_median_low.metadata)
```

```{python}
# After loading and reducing SIF
cube_SIF_original_median = cube_SIF_original_median.reduce_temporal(reducer="median")

# Check if SIF cube has bands dimension
print("SIF dimensions:", cube_SIF_original_median.metadata)
print("SIF process graph (last few steps):")
print(cube_SIF_original_median)
```
```{python}
"""
dataset_SIF_low.execute_batch(outputfile="openeo_sif_low.nc")
"""
```
...
Given the previous formulation we optimize the function using L-BFGS-B and a spatial moving window with 40 observations. The optimization function is defined in [sif_downscaling_openEO_CDSE/udf/udf_parameters_optim_low_res.py](sif_downscaling_openEO_CDSE/udf/udf_parameters_optim_low_res.py).
...
parameters_cube_low_rename.execute_batch(
    outputfile="data/results_parameters_optim_low_resolution.tif"
)
"""
```
