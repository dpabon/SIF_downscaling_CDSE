...
def apply_datacube(input_cube: xarray.Dataset, context: dict) -> xarray.DataArray:
    ...
    if any(
        local < min_obs for local in [SIF_w_valid, VI_w_valid, ET_w_valid, LST_w_valid]
    ):
        ...
        return xarray.DataArray(
            np.reshape(
                np.full(len(param_ini), np.nan, dtype=np.float32), shape=(1, 1, 1, 6)
            ),
            dims=["t", "y", "x", "bands"],
            coords=dict(t=coord_t, x=coord_x, y=coord_y, bands=output_bands),
        )
    ...
    return xarray.DataArray(
        np.reshape(optimized_params, (1, 1, 1, 6)),
        dims=["t", "y", "x", "bands"],
        coords=dict(t=coord_t, x=coord_x, y=coord_y, bands=output_bands),
    )
