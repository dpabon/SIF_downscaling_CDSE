...
regridder = xe.Regridder(
    parameters_low, lst_high, method="bilinear", extrap_method="nearest_s2d"
)
...
```{python}
# | eval: false
# cube_to_upscale.execute_batch(outputfile="data/sif_predictos_cube_high.nc")
```
Checking that everything is in order with the code to upscale
```{python}
cube_to_upscale_local = xr.open_dataset("data/sif_predictos_cube_high.nc")
