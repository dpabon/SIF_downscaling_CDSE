...
cube_NDVI = connection.load_collection(
...
cube_NDVI = cube_NDVI * 0.004 - 0.08
...
There is a NDVI product composite that looks quite well unfortunately it can not be combined with CDSE products here the ref to the [issue](https://forum.dataspace.copernicus.eu/t/combining-cdse-and-terrascope-products/4472)
...
cube_NDVI_median = cube_NDVI.reduce_temporal(reducer="median")
...
cube_LST_median_low = cube_LST_median.resample_cube_spatial(
    target=cube_SIF_original_median, method="near"
)
...
dataset_SIF_low.execute_batch(
    outputfile="data/sif_predictors_cube_low.nc",
    job_options={
        "split_strategy": {"crossbackend": {"method": "simple"}},
    },
)
"""
```
